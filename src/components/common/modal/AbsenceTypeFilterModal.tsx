...
export default function AbsenceTypeFilterModal({
   visible,
   setVisible,
   setStatusValue,
   statusValue,
}: InferProps<typeof AbsenceTypeFilterModal.propTypes>) {
    ...
    const handleSaveValue = () => {
        setStatusValue(
            LIST_ABSENCE_TYPE.find(item => item.value === currentFilter.value),
        );
        setVisible(false);
    };
    return (
        <ReactNativeModal
            ...
            <View style={styles.content}>
                <View style={styles.header}>
                    <Text style={[fs_14_700, text_red, text_center]}>LOẠI NGHỈ</Text>
                    <Pressable
                        hitSlop={10}
                        onPress={() => {
                            setVisible(false);
                ...
                <View style={styles.row_container}>
                    {LIST_ABSENCE_TYPE.map((item: any, index) => (
                        <View
                            key={index}
                            style={[
                                styles.row,
                                ...
                                {
                                    backgroundColor: LIST_ABSENCE_TYPE_COLOR[item.value],
                                },
                            ]}>
                            <PrimaryCheckbox
                                label={item.label}
...
AbsenceTypeFilterModal.propTypes = {
